...
class FirebaseMessageFetcher():
    ...
    def __init__(self,
        ...
        self.entity_uuid = credentials["entity_uuid"]
    ...
    def delete_message(self, timestamp):
        ...
        try:
            self.db.child("channels").child(self.entity_uuid).child(timestamp).remove(token=self.id_token)
        except HTTPError as e:
    ...
    def get_messages(self):
        ...
        try:
            rsp = self.db.child("channels").child(self.entity_uuid).get(token=self.id_token)
        except HTTPError as e:
            self.refresh_id_token()
            rsp = self.db.child("channels").child(self.entity_uuid).get(token=self.id_token)
    ...
    def start_streaming(self):
        self.stream = self.db.child("channels").child(self.entity_uuid).stream(self.stream_handler, token=self.id_token)
